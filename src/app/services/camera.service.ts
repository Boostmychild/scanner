...
export class CameraService {
   ...
   */
  private async checkWebCameraPermissions(): Promise<{
    granted: boolean;
    message: string;
  }> {
    try {
      if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
        return {
          granted: false,
          message:
            "Camera API not supported in this browser. Please use Chrome, Firefox, or Safari.",
        };
      }
      ...
      try {
        ...
        if (error.name === "NotAllowedError") {
          return {
            granted: false,
            message:
              'Camera permission denied. Please click "Allow" when prompted and refresh the page.',
          };
          ...
          return {
            granted: false,
            message:
              "Camera access blocked. Please ensure you are using HTTPS or localhost.",
          };
        } else {
          return {
            granted: false,
      ...
      return {
        granted: false,
        message:
          "Unable to check camera permissions. Please try refreshing the page.",
      };
    }
  }
  ...
  async startCamera(video: HTMLVideoElement): Promise<void> {
    ...
    try {
      ...
      if (error.name === "NotAllowedError") {
        ...
        userMessage =
          "No back camera found. Please ensure your device has a back-facing camera.";
      } else if (error.name === "NotReadableError") {
