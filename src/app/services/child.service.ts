...
export class ChildService {
  ...
  getChildren(filters?: ChildFilters): Observable<ChildrenResponse> {
    ...
    if (filters) {
      ...
      if (filters.school_id)
        params = params.set("school_id", filters.school_id.toString());
      ...
      if (filters.service_provider_id)
        params = params.set(
          "service_provider_id",
          filters.service_provider_id.toString(),
        );
      if (filters.search) params = params.set("search", filters.search);
      if (filters.has_media !== undefined)
        params = params.set("has_media", filters.has_media.toString());
    }
  ...
  /**
   * Get children filtered by school context (service provider and school)
   */
  getChildrenBySchool(
    serviceProviderId: number,
    schoolId: number,
    filters?: Omit<ChildFilters, 'service_provider_id' | 'school_id'>
  ): Observable<ChildrenResponse> {
    let params = new HttpParams()
      .set('service_provider_id', serviceProviderId.toString())
    ...
    return this.http.get<ChildrenResponse>(`${this.baseUrl}/children/by-school`, {
      params,
    });
  }
  ...
  getChildByStudentRefId(studentRefId: string): Observable<Child> {
    return this.http.get<Child>(
      `${this.baseUrl}/children/student/${studentRefId}`,
    );
  }
  ...
  updateChild(
    id: number,
    child: Partial<Child>,
  ): Observable<{ success: boolean; message: string; child: Child }> {
  ...
  getAge(child: Child): number {
    if (
      !child.child_born_year ||
      !child.child_born_month ||
      !child.child_born_date
    ) {
      return 0;
    }
